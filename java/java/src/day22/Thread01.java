...
public class Thread01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * > multi-thread 란? <
		 * - 여러개의 thread가 동시에 수행되는 프로그램
		 * 
		 * - CPU는 시간을 잘게 쪼개서 thread를 번갈아 수행하면서
		 * 	사용자 입장에서는 동시에 처리되는 듯한 효과를 보게 됨.
		 *	->실제로 동시에 처리하는 것은 아님
		 *
		 * - Thread는 각각 자신만의 작업공간을 가짐 (context)
		 * - 각 thread는 공유하는 자원이 있을 수 있음. (자바에서는 static instance)
		 * - 여러 thread가 공유하는 자원 중 경쟁이 발생하는 부분을 
		 * 		-> critical section(임계영역) 이라고 함
		...
		System.out.println(Thread.currentThread().getName()); //현재실행중인 Thread의 이름
